...
// the thread need to wait for traffic light "condp" before output to the
//buffer,
...
pthread_cond_init(&condc,0); // intialize the condition varible for
//consumer
pthread_cond_init(&condp,0); // initalize the condition varible for
//producer
pthread_create(&con, 0, consumer, 0); // create thread map to function
//consumer
pthread_create(&pro, 0, producer, 0); // create thread map to function
//producer
pthread_join(pro,0); // put the producer thread into the
//system
pthread_join(con,0); // put the consummer thread into the
//system
